...
if __name__ == "__main__":
    ...
    run_sql(url_prefix=url_prefix, save=False, headers=headers, json={'id': id, 'query': {'type': 'sql', 'sql': 'select count(*) from nyc_taxi'}})
    run_sql(url_prefix=url_prefix, save=False, headers=headers, json={'id': id, 'query': {'sql': 'SELECT count(*) AS countval FROM nyc_taxi', 'type': 'sql'}})
    run_sql(url_prefix=url_prefix, save=False, headers=headers, json={'id': id, 'query': {'type': 'sql', 'sql': 'SELECT MIN(fare_amount) as min , MAX(fare_amount) as max FROM nyc_taxi'}})
    ...
    run_sql(url_prefix=url_prefix, save=False, headers=headers, json={'id': id, 'query': {'sql': "SELECT ST_Point (dropoff_longitude, dropoff_latitude) AS point FROM nyc_taxi WHERE (ST_Within (ST_Point (dropoff_longitude, dropoff_latitude), ST_GeomFromText('POLYGON ((-73.5 40.1, -73.5 41.1, -70.5 41.1, -70.5 40.1, -73.5 40.1))'))) GROUP BY point", 'type': 'icon', 'params': {'width': 810, 'height': 465, 'icon': {'bounding_box': [-73.5, 40.1, -70.5, 41.1], 'coordinate_system': 'EPSG:4326', 'icon_path': '/tmp/taxi.png'}}}})
    run_sql(url_prefix=url_prefix, save=False, headers=headers, json={'id': id, 'query': {'type': 'sql', 'sql': 'SELECT dropoff_longitude, dropoff_latitude FROM nyc_taxi WHERE dropoff_longitude IS NOT NULL AND dropoff_latitude IS NOT NULL LIMIT 1'}})
